    ...
    if "Custom" in DOWHAT:
        ...
        for plot in plotsDQM:
            for i,t in enumerate(tracklistA):
                yrange="--yrange='0.7,1.0'"
                if 'duplicate' in plot: yrange="--yrange='0.0,0.1'"
                if 'fake' in plot: yrange="--yrange='0.01,1.0' --logy"
                if 'Iter2' in t and 'effic'in plot: yrange="--yrange='0.001,1.0' --logy"
                if 'Pt' in plot: yrange+=" --logx"
                
                legA="IterL3 (Ideal)"
                legB=",IterL3 (BadModules)"
                legC=",IterL3 (BadModules+Triplets)"
                legD=",IterL3 (BadModules+TripletsAndPairs)"
                legA=legA+legB+legC+legD
                plotname="{fld}_hltAssociatorByHits/{plot}".format(fld=t,plot=plot)
                plotA=plotname
                name="{trk}_{plot}".format(trk=t.replace("hlt",""),plot=plot)
                for i in range(len(IFILE.split(','))-1): 
                    if i==(len(IFILE.split(','))-1): plotA+=",hltIter3IterL3MuonMerged_hltAssociatorByHits/{plot}".format(plot=plot)
                    plotA+=","+plotname
                cmd = "python plotter.py --outdir {odir} --inputfile {ifile} --outname {name} --leg '{leg}' --dqm '{plot}' {yrange}".format(odir = ODIR, ifile=IFILE, name=name,leg=legA,plot=plotA,yrange=yrange )
                print "Running... "+ cmd
                os.system(cmd)
            ...
            if "Combo" in DOWHAT: 
                ...
                if len(IFILE.split(','))==2:
                    for i,t in enumerate(tracklistCombo2017):
                        ...
                        legB="2017"
                        if 'fake' in plot: yrange="--yrange='0.01,1.0' --logy"
                        if 'Iter2' in t and 'effic'in plot: yrange="--yrange='0.01,1.0' --logy"
                ...
                for i,t in enumerate(tracklistA):
                    ...
                    legA="IterL3 (v21 fix)"
                    legB="IterL3 (v21 fix - BadPixelModules)"
                    plotA="{fld}_hltAssociatorByHits/{plot}".format(fld=tracklistA[i],plot=plot)
                    name="{trk}_{plot}".format(trk=tracklistA[i].replace("hlt",""),plot=plot)
                    cmd = "python plotter.py --outdir {odir} --inputfile {ifile} --outname {name} --leg '{legA},{legB}' --dqm '{plotA},{plotB}' {yrange}".format(odir = ODIR, ifile=IFILE, name=name,legA=legA,legB=legB,plotA=plotA,plotB=plotA,yrange=yrange )
                    print "Running... "+ cmd
                    os.system(cmd)
...
##                cmd = "python plotter.py --outdir {odir} --inputfile '{ifile}' --outname {name} --leg '{legA},{legB},{legC}' --title 'L3/L2 efficiency' --eff '{plot}_num,{plot}_den' {yrange}".format(odir = ODIR, ifile=IFILE, name=name,legA=legA,legB=legB,legC=legC,plot=plot,yrange=yrange )
    ...
    if "MuonNtuple" in DOWHAT:
        for plot in plotsEff:
            ...
            legB="FromL2+FromL1"
            name="{plot}".format(plot=plot)
            cmd = "python plotter.py --outdir {odir} --inputfile '{ifile}' --outname {name} --leg '{legA},{legB}' --title 'L3/L1 efficiency' --draw '{plot}' {yrange}".format(odir = ODIR, ifile=IFILE, name=name,legA=legA,legB=legB,plot=plot,yrange=yrange )
            print "Running... "+ cmd
            os.system(cmd)

    if "MuonNTupleMitigation" in DOWHAT:
        for plot in plotsEff:
            if "Displaced" in IFILE:
                yrange="--yrange='0.01,1.0'"
            else:
            ...
            if "TurnOn" in plot:  yrange="--yrange='0.01,1.0'"
            if "Eff" in plot: yrange="--yrange='0.97,1.0'"
            
            legA="Ideal,BadPixelModules"
            legB="BadPixelModules+Triplets"
            legC="BadPixelModules+Triplets+Iter3(Pairs)"
            name="{plot}".format(plot=plot)
            cmd = "python plotter.py --outdir {odir} --inputfile '{ifile}' --outname {name} --leg '{legA},{legB},{legC}' --title 'L3/L1 efficiency' --draw '{plot}' {yrange}".format(odir = ODIR, ifile=IFILE, name=name,legA=legA,legB=legB,legC=legC,plot=plot,yrange=yrange )
            print "Running... "+ cmd
            os.system(cmd)
